...
void Inorder(Dugum* nptr)   //Test icin inorder kullandým. Testi gecti 
{
    if (nptr) {
        Inorder(nptr->sol);
        cout << nptr->veri << endl;
...
int main()
{
    // asaðýda aðaclar sýrasýyla tanýmlanmýþtýr.
    ...
    //Inorder(ikinciAgac->kok);  // test icin kullanýlan fonksiyon.
    bool bulduMu;
    int secim;
    int secim2;
    int veri;
...
bool IkiliSiralamaAgaci::DFS(Dugum* dugum, int aranacak_veri)
{
    ...
    while (!yigin.empty())
    {
        ...
        if (aranacak_veri == dugum->veri)
        {
            ...
            return 1;//veriyi bulmasý durumunda '1' yani true donecek
        }
    ...
    Sure = elapsed.count();
    cout << "gecen sure : " << Sure << endl;
    return 0;//veriyi bulamamasý durumunda '0' yani false donecek
}
...
bool IkiliSiralamaAgaci::BFS(Dugum* dugum, int aranacak_veri)
{
    ...
    while (!kuyruk.empty())
    {
        ...
        if (aranacak_veri == dugum->veri)
        {
            ...
            Sure = elapsed.count();
            cout << "gecen sure : " << Sure << endl;
            return 1;//veriyi bulmasý durumunda '1' yani true
        }
    ...
    Sure = elapsed.count();
    cout << "gecen sure : " << Sure << endl;
    return 0;//veriyi bulamamasý durumunda '0' yani false doner.
}